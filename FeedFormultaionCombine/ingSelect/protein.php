...
for($p=0; $p< $N; $p++)
{
array_push($boxArray,$a[$p]);
    //echo($a[$p] . " , ");
}
}
$ingBox = implode("Q", $boxArray);
$_SESSION['ingBox'] = $ingBox;
...
$query = "SELECT ing_name FROM ingredient WHERE category='P'";
mysqli_query($db, $query) or die('Error querying database.');

$query1="SELECT CP FROM majorcontent WHERE ing_name IN (SELECT ing_name FROM ingredient WHERE category='P')";
mysqli_query($db, $query1) or die('Error querying database.');


$query2="SELECT tip FROM ingredient WHERE category='P'";
...
$result2=mysqli_query($db, $query2);
 ...
 echo'<form action="../animalselection/feedcalculator/feedCalc.php?ageid='.$ageid.'" method="post">';
$c=count($ingredients);
 for($j=0;$j<$c; $j++){
  ...
  <div id=inner style="background-color:#5A69A6;
   width:'.$content[$j].'%; 
   height:10px;
